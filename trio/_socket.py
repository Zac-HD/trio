...
@_add_to_all
def socket(family=_stdlib_socket.AF_INET, type=_stdlib_socket.SOCK_STREAM,
           proto=0, fileno=None):
...
class _SocketType(SocketType):
    ...
    async def bind(self, address):
        ...
        if (
            hasattr(_stdlib_socket, "AF_UNIX") and
            self.family == _stdlib_socket.AF_UNIX and address[0]
        ):
    ...
    async def connect(self, address):
        ...
        err = self._sock.getsockopt(_stdlib_socket.SOL_SOCKET,
                                    _stdlib_socket.SO_ERROR)
        if err != 0:
